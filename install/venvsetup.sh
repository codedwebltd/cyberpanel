...
sed -i 's|git clone https://github.com/codedwebltd/cyberpanel|echo downloaded|g' install.py
...
sed -i 's|git clone https://github.com/codedwebltd/cyberpanel|wget https://cyberpanel.sh/cyberpanel-git.tar.gz \&\& tar xzvf cyberpanel-git.tar.gz|g' install.py
...
wget -O requirements.txt https://raw.githubusercontent.com/codedwebltd/cyberpanel/main/requirments.txt
...
if [[ $DEV == "ON" ]] ; then
	#install dev branch 
	#wget https://raw.githubusercontent.com/codedwebltd/cyberpanel/$BRANCH_NAME/requirments.txt
	cd /usr/local/
	python3.6 -m venv CyberPanel
	source /usr/local/CyberPanel/bin/activate
	wget -O requirements.txt https://raw.githubusercontent.com/codedwebltd/cyberpanel/$BRANCH_NAME/requirments.txt
...
if [[ $SERVER_COUNTRY == "CN" ]] ; then
	...
	if [[ $DEV == "ON" ]] ; then
	git clone https://github.com/codedwebltd/cyberpanel cyberpanel
	cd cyberpanel
